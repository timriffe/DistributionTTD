...
par(mai=c(.5,.5,.2,1))
LexisMap(sqrt(Var),log=FALSE,contour=TRUE,colramp=colramp,nbreaks=15,LexRef=FALSE,useRaster=TRUE)
dev.off()

pdf("Figures/Surf/SskewSWEf.pdf",height=5,width=11)
par(mai=c(.5,.5,.2,1))
LexisMap(Sskew,log=FALSE,contour=TRUE,colramp=colramp,nbreaks=11,LexRef=FALSE,useRaster=TRUE)
dev.off()

pdf("Figures/Surf/KurtSWEf.pdf",height=5,width=11)
#dev.new(height=5,width=11)
par(mai=c(.5,.5,.2,1))
LexisMap(Kurt,log=FALSE,contour=TRUE,colramp=colramp,nbreaks=11,LexRef=FALSE,useRaster=TRUE)
dev.off()

pdf("Figures/Surf/CVSWEf.pdf",height=5,width=11)
# dev.new(height=5,width=11)
par(mai=c(.5,.5,.2,1))
LexisMap(CV,log=FALSE,contour=TRUE,colramp=colramp,nbreaks=15,LexRef=FALSE,useRaster=TRUE)
dev.off()
...
#X <- apply(X,1,sort)
#library(lattice)
#pdf("Surface.pdf",width=8,height=4)
